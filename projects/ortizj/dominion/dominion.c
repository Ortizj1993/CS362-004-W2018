...
int adventurerEffect(struct gameState* state , int currentPlayer, int z, int cardDrawn, int* temphand, int drawntreasure)
{
  ...
  drawCard(currentPlayer, state);
  	...
  	drawntreasure++; // bug for adventurer +=2 , original ++
  else{
...
int FeastEffect(struct gameState* state, int currentPlayer, int choice1, int *temphand)
{
	...
	state->hand[currentPlayer][i] = -1;//Set to nothing
      ...
      updateCoins(currentPlayer, state, 5);// bug changed update coins to 6 is bug from 5 is original
...
int VillageEffect(struct gameState* state, int currentPlayer, int handPos)
{
  ...
  //+2 Actions
  state->numActions = state->numActions + 2; // changed number of actions to 4 is bug from 2 is orginal so there is two extra actions the player gets
...
return -1;
  ...
  gainCard(choice2, state, 2, 0);// changed current player from gaining card to 0
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    {
    case adventurer:
     return adventurerEffect(state,currentPlayer,z,cardDrawn,temphand,drawntreasure);
    ...
    return FeastEffect(state, currentPlayer, choice1, temphand);
